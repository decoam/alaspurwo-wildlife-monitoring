...
export async function getDashboardData(search = "") {
  try {
    ...
    const safeSearch = rawSearch ? escapeRegExp(rawSearch) : "";

    const recentFilter = safeSearch
      ? {
          $or: [
    ...
    const [statsResult, recentResult, weeklyResult, categoryResult] =
      await Promise.all([
    ...
    const recentObservations: RecentObservationItem[] = (
      recentResult as Array<Record<string, unknown>>
    ).map((item) => {
      const createdAt =
        item.createdAt instanceof Date
          ? item.createdAt
          : new Date(item.createdAt as string | number);
      ...
      return {
        ...
        tanggalPengamatan: item.tanggalPengamatan
          ? new Date(item.tanggalPengamatan as string | Date).toISOString()
          : "",
    ...
    const dashboardStats: DashboardStats = {
      ...
      weeklyActivity: buildWeeklyActivity(
        weeklyResult as Array<{ _id: number; count: number }>
      ),
      categoryBreakdown: buildCategoryBreakdown(
        categoryResult as Array<{ _id: string; count: number }>
      ),
    };

    const session = await auth();
    const sessionUser = ((session as unknown as { user?: unknown } | null | undefined)
      ?.user ?? undefined) as
      | {
          email?: string;
          posPengamatan?: string;
    ...
    // Return fallback data agar dashboard tetap dapat merender UI tanpa leak DB error
    return {
      stats: {
        totalObservations: 0,
        observationsToday: 0,
      ...
      error: "Gagal memuat data dashboard. Silakan coba beberapa saat lagi.",
    };
  }
}
