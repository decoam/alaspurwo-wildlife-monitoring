...
export function ObservationTable({ items, deleteAction }: ObservationTableProps) {
          ...
          <tbody className="divide-y divide-emerald-900/60 bg-[#0d1d14]">
            {items.map((item) => (
              <tr key={item._id} className="transition hover:bg-emerald-950/30">
                ...
                <td className="px-4 py-3">{new Date(item.tanggalPengamatan).toLocaleDateString("id-ID")}</td>
                ...
                <td className="px-4 py-3">
                  <div className="flex items-center gap-2">
                    <Link href={`/dashboard/observations/${item._id}`} className="rounded-full border border-emerald-900/60 p-2 text-emerald-200 transition hover:bg-emerald-900/60" title="Detail">
                      <Eye className="h-4 w-4" />
                    </Link>
                    <Link href={`/dashboard/observations/edit/${item._id}`} className="rounded-full border border-emerald-900/60 p-2 text-sky-200 transition hover:bg-emerald-900/60" title="Edit">
                    ...
                    <form action={deleteAction}>
                      ...
                      <button
                        type="submit"
                        onClick={(event) => {
                          if (!window.confirm("Apakah Anda yakin ingin menghapus data pengamatan ini?")) {
